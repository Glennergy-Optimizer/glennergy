 ...
 *
 * Fetches, parses, and stores spot price data for Swedish electricity
 * regions (SE1–SE4). Current behavior fetches both current and next day.
 *
 * @note Internally performs blocking network I/O and JSON parsing.
 * @{
 */
 ...
 *
 * Contains both parsed entries and a raw JSON copy of the upstream response.
 * The entry array is sized for two days of quarter-hour data (96 * 2).
 *
...
typedef struct
{
    ...
    SpotPriceEntry kvartar[192];/**< Parsed entries; only first `count` are valid. */
    char raw_json_data[32000];  /**< Raw JSON data from API. */ // Vid ett test av spotprisdatan var den 13600 
 ...
 *
 * @param d Pointer to daily spot price data to print.
 */
 ...
 *
 * @param a Pointer to all spot price data to print.
 */
 ...
 *
 * Fetches data for both current and next day from an external API, parses
 * JSON, and populates the provided structure.
 *
